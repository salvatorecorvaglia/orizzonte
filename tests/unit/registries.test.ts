...
/**
 * Cached version lookups must not cross registries.
 *
 * `TtlCache` is backed by `globalState`, which VS Code shares across every
 * workspace in the install — so an entry that does not name the registry it
 * came from is served to a project configured for a different one. That is both
 * wrong (switching an override on or off keeps serving the previous registry
 * until the TTL lapses) and leaky (a private mirror's package names and
 * versions surface in an unrelated workspace). Node and Python always keyed on
 * it; these five did not.
 */
describe('cache keys separate registries', () => {
  /** Two contexts sharing one cache, pointed at different registries. */
  function sharedCache(ecosystem: string, body: unknown) {
    const base = makeContext();
    const calls: string[] = [];
    const make = (override: string | undefined) => ({
      ...base,
      http: {
        ...base.http,
        getJson: (url: string) => {
    ...
    return {
      calls,
      privateCtx: make('https://mirror.internal'),
      publicCtx: make(undefined),
    };
  }
  ...
  const cases = [
    {
      ...
      run: (ctx: never) => new CargoProvider().fetchVersions(['serde'], ctx),
    },
    {
      name: 'Composer',
      ecosystem: 'composer',
      ...
      run: (ctx: never) =>
        new ComposerProvider().fetchVersions(['vendor/pkg'], ctx),
    },
    {
      name: 'Go',
      ecosystem: 'golang',
      body: { Version: 'v1.0.0' },
      run: (ctx: never) =>
        new GoProvider().fetchVersions(['github.com/a/b'], ctx),
    },
    {
      name: 'Maven',
      ecosystem: 'maven',
      body: { response: { docs: [{ v: '1.0.0' }] } },
      run: (ctx: never) =>
        new MavenProvider().fetchVersions(['org.example:lib'], ctx),
    },
  ];

  it.each(cases)(
    '$name refetches rather than serving another registry’s cached answer',
    async ({ ecosystem, body, run }) => {
      const { calls, privateCtx, publicCtx } = sharedCache(ecosystem, body);

      await run(privateCtx as never);
      const afterPrivate = calls.length;
      expect(afterPrivate).toBeGreaterThan(0);

      // Same package, same shared cache, different registry: the public
      // registry must actually be asked, not handed the mirror's entry.
      await run(publicCtx as never);

      expect(calls.length).toBeGreaterThan(afterPrivate);
      expect(calls.at(-1)).not.toMatch(/mirror\.internal/);
    },
  );

  it('still serves a cached answer for the same registry', async () => {
    const { calls, privateCtx } = sharedCache('cargo', {
      crate: { name: 'serde', newest_version: '1.0.0' },
      versions: [{ num: '1.0.0', yanked: false }],
    });

    await new CargoProvider().fetchVersions(['serde'], privateCtx as never);
    const afterFirst = calls.length;

    await new CargoProvider().fetchVersions(['serde'], privateCtx as never);

    // The point of the key is separation, not defeating the cache entirely.
