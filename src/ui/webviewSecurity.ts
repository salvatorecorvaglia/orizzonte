...
export function buildContentSecurityPolicy(
  ...
  return [
    `default-src 'none'`,
    // No webview renders a remote image — the only `<img>` in either surface is
    // the logo, served from `localResourceRoots`. `https:` therefore bought
    // nothing and cost something: combined with the `'unsafe-inline'` styles
    // below, it is a channel for CSS to signal out to an arbitrary host.
