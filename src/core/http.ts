 ...
 *
 * A count alone is the wrong unit for a cache of response *bodies*: npm
 * packuments for popular packages run to several megabytes each, so 500 of
 * them is a budget of "somewhere between 5 MB and 2 GB" depending entirely on
 * which packages the workspace happens to use. Bytes bound the thing that
...
class HostLimiter {
  ...
  /**
   * Waits for this host's next slot.
   *
   * Cancellation matters more here than anywhere else in the client: crates.io
   * is capped at one request per second, so a cancelled scan of a 200-crate
   * project used to go on draining its queue for another three minutes —
   * holding the spinner up and issuing requests on behalf of a question nobody
   * is still asking. A waiter that aborts leaves the queue instead.
   */
...
export class HttpClient {
  ...
  /** Running total of `etagCache`'s body sizes, so eviction need not re-add them. */
  private etagBytes = 0;
  ...
  private async request(url: string, options: HttpOptions): Promise<string> {
    await this.waitForSlot(url, options.signal);
    /*
   ...
   * precisely backwards.
   */
  private async waitForSlot(url: string, signal?: AbortSignal): Promise<void> {
  ...
  private async attempt(
    ...
    deadline: number,
    /**
     * Set on the one retry that follows an unmatchable 304 — see below. It
     * suppresses the conditional headers so the server has to send a body.
     */
    skipConditional = false,
  ): Promise<string> {
    const cacheKey = etagCacheKey(url, options.headers);
    const cached =
      options.noCache || skipConditional
        ? undefined
        : this.etagCache.get(cacheKey);
    ...
    try {
      ...
      if (response.status === 304) {
        ...
        /*
         * A 304 we cannot honour: the entry we validated against was evicted
         * between sending `If-None-Match` and the answer coming back, which a
         * scan large enough to cycle the cache does routinely. The server is
         * right that nothing changed, but we no longer hold the body it is
         * referring to — so ask again unconditionally rather than falling
         * through to the `!response.ok` branch below and reporting a 304 as a
         * failed request.
         */
        if (!skipConditional) {
          return this.attempt(url, options, retryCount, deadline, true);
        }
      }
      ...
      if (!options.noCache) {
        ...
        if (etag || lastModified) {
          this.rememberResponse(cacheKey, { etag, lastModified, body });
        }
      }
  ...
  /**
   * Stores a revalidatable response, evicting the least recently stored until
   * the cache is back within both of its bounds. Insertion order is the
   * eviction order: `Map` preserves it, and a re-store deletes first so the
   * entry moves to the end.
   */
  private rememberResponse(key: string, entry: CachedEntry): void {
    this.forget(key);

    // A single response larger than the whole budget is not worth evicting
    // everything else to hold.
    if (entry.body.length > MAX_CACHED_BYTES) return;

    this.etagCache.set(key, entry);
    this.etagBytes += entry.body.length;

    while (
      this.etagCache.size > MAX_CACHED_RESPONSES ||
      this.etagBytes > MAX_CACHED_BYTES
    ) {
  ...
  /** Drops one entry, keeping the byte total honest. */
  private forget(key: string): void {
    const existing = this.etagCache.get(key);
    if (!existing) return;
    this.etagBytes -= existing.body.length;
...
/**
 * The ETag cache key: the URL, plus which credentials asked for it.
 *
 * Keying on the URL alone let one private registry's authenticated response be
 * revalidated and returned for an unauthenticated request to the same URL, and
 * for a request carrying a *different* user's token. The header is hashed
 * rather than stored so the key never carries the credential itself.
 */
function etagCacheKey(
  url: string,
